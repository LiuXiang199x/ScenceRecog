...
# save_path = sys.argv[2]
    ...
    classes = list()
    with open(txt) as class_file:
        for line in class_file:
            cur_sn = line.split()[0].rsplit('/')[-1].strip().rsplit('_')[0] + '_' + \
                     line.split()[0].rsplit('/')[-1].strip().rsplit('_')[1]
            if not cur_sn in classes:
                classes.append(cur_sn)
    classes = tuple(classes)
    file = open('/media/agent/eb0d0016-e15f-4a25-8c28-0ad31789f3cb/Scene/DEDUCE/download_classes.txt', 'w')
