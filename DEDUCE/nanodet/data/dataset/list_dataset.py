...
class LISTDataset(CocoDataset):
    ...
    def xml_to_coco(self, ann_path):
        ...
        for idx, line in enumerate(ann_lines):
            ...
            height, width = int(fd[1].split(',')[1]), int(fd[1].split(',')[0])
            info = {
                "file_name": file_name,
                "height": height,
                "width": width,
            ...
            for _object in fd[2:]:
                ...
                if len(fd1) < 5:
                    continue
                cat_id = int(fd1[-1]) + 1
                xmin = int(float(fd1[0]))
                ymin = int(float(fd1[1]))
                xmax = int(float(fd1[2]))
                ymax = int(float(fd1[3]))
                w = xmax - xmin
                h = ymax - ymin
                if w < 0 or h < 0:
                    logging.warning(
