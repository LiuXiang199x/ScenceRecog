# python test_combined_v2.py --dataset=data --envtype=home --config config/nanodet_irbt_list_dataset.yml --model workspace/nanodet-plus-m-1.5x_416/model_best/nanodet_model_best.pth
...
model_file = '%s_best_combined_v4.pth.tar' % arch
...
elif(args.dataset == '35W'):
    valdir = '/mnt/sdc/zhuhao/scene_dataset/35W/2'
...
predictor = Predictor(cfg, args.model, logger, device="cuda:7")

txt_path_door = "1_door.txt"
txt_path_bed = "1_bed.txt"
txt_path_drawing = "1_sofa_tv_tea.txt"
txt_path_dinning = "1_dinning_table.txt"
txt_path_toilet = "1_toilet.txt"
txt_path_other = "1_other.txt"

file_txt_door = open(txt_path_door, "w")
file_txt_bed = open(txt_path_bed, "w")
file_txt_drawing = open(txt_path_drawing, "w")
file_txt_dinning = open(txt_path_dinning, "w")
file_txt_toilet = open(txt_path_toilet, "w")
file_txt_other = open(txt_path_other, "w")

detailed_count = torch.zeros(6)
num_badImag = 0
count = 0
print("count", count)
for img_name in os.listdir(valdir):
    count += 1
    if count % 500 == 0:
        print("count", count)
    img_dir = os.path.join(valdir, img_name)
    try:
    ...
    input_txtname = img_name + "\n"
    if obj_hot_vector[5] == 1:
        file_txt_door.write(input_txtname)
        detailed_count[0] += 1
    elif obj_hot_vector[5] == 0 & obj_hot_vector[0] == 1:
        file_txt_bed.write(input_txtname)
        detailed_count[1] += 1
    elif obj_hot_vector[5] == 0 & obj_hot_vector[7] + obj_hot_vector[8] + obj_hot_vector[10] > 0:
        file_txt_drawing.write(input_txtname)
        detailed_count[2] += 1
    elif obj_hot_vector[5] == 0 & obj_hot_vector[4] == 1:
        file_txt_dinning.write(input_txtname)
        detailed_count[3] += 1
    elif obj_hot_vector[5] == 0 & obj_hot_vector[9] == 1:
        file_txt_toilet.write(input_txtname)
        detailed_count[4] += 1
    else:
        file_txt_other.write(input_txtname)
        detailed_count[5] += 1

# print('Average test accuracy is = {:2.2f}%'.format(sum(accuracies_list)/len(accuracies_list)))

file_txt_dinning.close()
file_txt_bed.close()
file_txt_door.close()
file_txt_drawing.close()
file_txt_toilet.close()
file_txt_other.close()
print("detailed_count", detailed_count)
print("num_badImag", num_badImag)
